...
if ($requri != 'install' && $requri != NULL && substr($requri, - 1) != "!" && !$post_values['adminProceed'] && $reqhash == 'raw') {
...
if ($requri != 'install' && $post_values['submit']) {
    ...
    $paste = array('ID' => $pasteID, 'Author' => $bin->checkAuthor($post_values['author']), 'IP' => $_SERVER['REMOTE_ADDR'], 'Lifespan' => $post_values['lifespan'], 'Protect' => $post_values['privacy'], 'Parent' => $requri, 'Content' => $post_values['pasteEnter']);
    ...
    if (strlen($post_values['pasteEnter']) > 10 && mb_strlen($paste['Content']) <= $SPB_CONFIG['max_bytes'] && $db->insertPaste($paste['ID'], $paste)) {
    ...
    if ($requri) {
        ...
        echo '<div id="adminFunctions">
							<form id="adminForm" action="' . $bin->linker($requri) . '" method="post">
								...
								<input id="adminPass" type="password" name="adminPass" value="' . $post_values['adminPass'] . '" />
								<br /><br />
